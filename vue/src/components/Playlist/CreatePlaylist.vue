<template>
    <v-text-field label="Title" placeholder="Title" v-model="playlistBeingCreated.title"></v-text-field>
    ...
    <div class="d-flex w-100 justify-end mt-6">
        <v-btn @click="createPlaylist" :disabled="!playlistBeingCreated.title?.trim().length" variant="outlined">create</v-btn>
    </div>
</template>
...
const { createPlaylist } = useVideoStore()

const videos = playlistBeingCreated.value?.videos
const search = ref("")
...
function addVideo(id) {
    playlistBeingCreated.value.videos.unshift(id)
}
function removeVideo(id) {
    const index = videos.findIndex(v=> v === id)
    playlistBeingCreated.value.videos.splice(index, 1)
}
